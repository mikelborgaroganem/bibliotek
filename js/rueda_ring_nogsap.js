...
(function(){
  ...
  const DBG_WHEEL = true; const wdbg = (...args)=>{ try{ console.log('[WHEEL]', ...args);}catch(_){}};

  function rotateTo(angleDeg){
    wdbg('rotateTo', {angleDeg, target: -(angleDeg + ANGLE_OFFSET)});
  ...
  function setSelectFromSeg(seg){
    ...
    if(sel && seg){ sel.value = seg.id; wdbg('dispatch change from wheel, value=', sel.value); sel.dispatchEvent(new Event('change')); }
  }
  ...
  function handleSelectBySeg(seg){
    wdbg('handleSelectBySeg', seg ? {id: seg.id, angle: parseFloat(seg.getAttribute('data-angle')) || 0} : null);
    if(!seg) return;
