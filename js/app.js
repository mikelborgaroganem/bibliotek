...
function applyFilters(){
  ...
  FILTERED = RAW.filter(r => {
    const okF = !f || norm(r.funnel) === f || norm(r.funnel).toLowerCase() === f.toLowerCase();
  ...
  render(FILTERED); // will be empty initially
}
...
async function load(){
  ...
  RAW = rows;
  FILTERED = []; // start empty until user selects funnel
  render(FILTERED); // will be empty initially
