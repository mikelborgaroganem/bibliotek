...
let RAW = [];
let FILTERED = [];
const REQUIRE_FUNNEL_SELECTION = true;
const DEBUG = true;
const dbg = (...args) => { try { console.log('[DBG]', ...args); } catch(_){} };


function isEmptyFunnel(v){
  const s = (v||'').toString().trim().toLowerCase();
  return !s || s === 'todos' || s === 'all' || s === '*' || s === '--' || s === 'selecciona' || s === 'selecciona un sector' || s === 'select';
}
...
function applyFilters(){
  ...
  const attrib = (els.attrib && els.attrib.value ? els.attrib.value : '').trim();
  const platform = (els.platform && els.platform.value ? els.platform.value : '').trim();
  const q = (els.q && els.q.value ? els.q.value : '').trim();

  if (typeof dbg === 'function') dbg('applyFilters() start', {f, attrib, platform, q});

  if (REQUIRE_FUNNEL_SELECTION && (!f || f.toLowerCase()==='todos' || f.toLowerCase()==='all')) {
    FILTERED = [];
    if (typeof dbg === 'function') dbg('applyFilters gated: empty or TODOS/ALL');
    render(FILTERED);
    return;
  }

  const norm = (s) => (s || '').toString().trim().toLowerCase();
  const includes = (haystack, needle) => norm(haystack).indexOf(norm(needle)) !== -1;

  const get = (row, keys) => { for (const k of keys){ if (row && Object.prototype.hasOwnProperty.call(row, k)) return row[k]; } return ''; };

  const funnelSyn = {
    'awareness': ['awareness', 'alcance'],
    'consideration': ['consideration', 'consideración', 'consideracion'],
    'conversion': ['conversion', 'conversión', 'conversiones'],
    'loyalty': ['loyalty', 'fidelización', 'fidelizacion', 'retention', 'retención', 'retencion']
  };
  function selectedSet(sel){
    const s = norm(sel);
    for (const k in funnelSyn){ if (funnelSyn[k].includes(s)) return new Set(funnelSyn[k]); }
    return new Set([s]);
  }
  const target = selectedSet(f);

  function funnelMatches(cellValue){
    const tokens = norm(cellValue).split(/[,/|;–-]+/).map(t => t.trim()).filter(Boolean);
    const ok = tokens.some(t => target.has(t));
    if (typeof dbg === 'function') dbg('funnelMatches', {cell: cellValue, tokens, target: Array.from(target), ok});
    return ok;
  }

  FILTERED = RAW.filter(r => {
    const rf = get(r, ['funnel','Funnel','FUNNEL']);
    const ra = get(r, ['attribution','Attribution','ATTRIBUTION']);
    const rp = get(r, ['platform','Platform','PLATFORM']);
    const fmt = get(r, ['format','Format','FORMAT']);
    const obj = get(r, ['objective','Objective','OBJECTIVE']);
    const kpi = get(r, ['kpi','KPI']);
    const spec = get(r, ['creative_specs','Creative_Specs','creative specs','Creative specs','CREATIVE_SPECS']);
    const note = get(r, ['notes','Notes','NOTES']);

    const okF = funnelMatches(rf);
    const okA = !attrib || norm(ra) === norm(attrib);
    const okP = !platform || includes(rp, platform);

    const blob = [fmt,obj,kpi,spec,rp,note].map(x => x || '').join(' ').toLowerCase();
    const okQ = !q || blob.indexOf(norm(q)) !== -1;

    const keep = okF && okA && okP && okQ;
    if (typeof dbg === 'function') dbg('row check', {fmt, rf, keep});
    return keep;
  });

  if (typeof dbg === 'function') dbg('applyFilters() done: count', FILTERED ? FILTERED.length : 0);
  render(FILTERED);
}
  ...
  FILTERED = RAW.filter(r => {
    const okF = !f || norm(r.funnel) === norm(f);
    const okA = !attrib || norm(r.attribution) === norm(attrib);
    const okP = !platform || includes(r.platform, platform);
    const blob = [r.format, r.objective, r.kpi, r.creative_specs, r.platform, r.notes].map(x => x || '').join(' ').toLowerCase();
    const okQ = !q || blob.indexOf(norm(q)) !== -1;
  ...
  if (REQUIRE_FUNNEL_SELECTION && isEmptyFunnel(els.funnel.value)) { dbg('applyFilters gated: empty funnel'); FILTERED = []; dbg('applyFilters() done: count', FILTERED ? FILTERED.length : 0);
  render(FILTERED); return; }
  console.log('applyFilters called, funnel value =', els.funnel.value);
  if (REQUIRE_FUNNEL_SELECTION && isEmptyFunnel(els.funnel.value)) { FILTERED = []; dbg('applyFilters() done: count', FILTERED ? FILTERED.length : 0);
  render(FILTERED); return; }
  if (REQUIRE_FUNNEL_SELECTION && isEmptyFunnel(els.funnel.value)) { FILTERED = []; dbg('applyFilters() done: count', FILTERED ? FILTERED.length : 0);
  render(FILTERED); return; }
  const f = els.funnel.value.trim();
  if (REQUIRE_FUNNEL_SELECTION && !f) {
    FILTERED = [];
    dbg('applyFilters() done: count', FILTERED ? FILTERED.length : 0);
  render(FILTERED);
    return;
  }
  FILTERED = RAW.filter(r => {
...
async function load(){
  ...
  dbg('Papa.parse complete, rows:', rows ? rows.length : 'n/a');
  RAW = rows;
els.funnel.value = '';
  try {
    const opts = Array.from(els.funnel.options).map(o => ({text:o.textContent, value:o.value, selected:o.selected}));
    dbg('funnel options at load', opts);
    dbg('funnel value after init', els.funnel.value);
  } catch(e) { dbg('error reading funnel options', e); }
FILTERED = [];
dbg('applyFilters() done: count', FILTERED ? FILTERED.length : 0);
  render(FILTERED);
...
els.btnClear.addEventListener('click', () => { els.funnel.value = '';
  try {
    const opts = Array.from(els.funnel.options).map(o => ({text:o.textContent, value:o.value, selected:o.selected}));
    dbg('funnel options at load', opts);
    dbg('funnel value after init', els.funnel.value);
  } catch(e) { dbg('error reading funnel options', e); } els.attrib.value = ''; els.platform.value = ''; els.q.value = ''; applyFilters(); });
...
document.addEventListener('DOMContentLoaded', () => {
  try {
    ...
    const opts = f ? Array.from(f.options).map(o => ({text:o.textContent, value:o.value, selected:o.selected})) : null;
    dbg('DOMContentLoaded -> funnel value', f && f.value, 'options', opts);
  } catch(e) { dbg('DOMContentLoaded error', e); }
});
